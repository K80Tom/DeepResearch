...
def collect_tool_calls(messages) -> tuple[list, list]:
    tools = [] # 保存工具名字
    tool_outputs = [] # 保存工具输出内容
    for msg in messages: # 遍历每一条消息。
        tool_calls = getattr(msg, "tool_calls", None)   # 从 msg 这个对象里尝试取出 tool_calls 属性；如果没有这个属性，就返回 None。
        if tool_calls:
...
def _invoke_json_agent(state: ResearchState, prompt: str, agent, agent_name: str, node: str, fallback: dict) -> tuple[dict, str, list]:
    human = HumanMessage(content=with_memory_context(state, prompt))    # 组合提示词与历史内容
    ...
    result = agent.invoke({"messages": [human]})    # 可能返回结果result = {"messages": [HumanMessage(content="用户提示词..."),AIMessage(content="模型回复...", tool_calls=[...]),ToolMessage(content="工具返回结果...", name="工具名"),AIMessage(content="最终回复...")]}
...
# 这个函数 = AI 的自动搜素引擎助手
# 你问一句话 → 它自动帮你扩展成 6 条精准搜索词让 AI 能查到最全、最准的资料！
...
def _is_query_grounded(candidate: str, user_query: str) -> bool:
    ...
    if not candidate_terms or not user_terms:
        return False
    # 4. 如果用户问题有核心实体（如AI、Python），并且出现在检索词里 → 直接判定相关
...
# 从用户问题生成检索词 从大纲每个章节里提取检索词 去重、过滤无效内容 最多返回 6 条，给 AI 去搜索资料(比较重要)
...
def _estimate_relevance(query: str, text: str) -> float:
    ...
    if not terms:
        return 0.0
    haystack = text.lower()
    #看 terms 里的词，有多少个出现在 haystack 里。
...
def web_search_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    # 这行代码是在清洗搜索结果，把缺少标题、摘要或链接的无效网页记录过滤掉。
